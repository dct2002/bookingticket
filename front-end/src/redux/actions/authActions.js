...
import {
  ...
  AUTH_GETALLUSERBYTIME_REQUEST,
  AUTH_GETALLUSERBYTIME_SUCCESS,
  AUTH_GETALLUSERBYTIME_FAIL,
...
export const getAllUserByTime = () => async (dispatch) => {
  try {
    dispatch({ type: AUTH_GETALLUSERBYTIME_REQUEST });
    const { data } = await axios.get(`${baseURL}/api/v1/auth/user/filter`);
    dispatch({
      type: AUTH_GETALLUSERBYTIME_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: AUTH_GETALLUSERBYTIME_FAIL,
      payload: error,
    });
  }
};
