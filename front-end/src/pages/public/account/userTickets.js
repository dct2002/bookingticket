...
function UserTickets() {
  ...
  const handleDeleteTicket = async (id) => {
    const reservation = reservations.filter((item) => item._id === id)[0];
    console.log(reservation.paymentId);
    await dispatch(
      deleteTicket(id,reservation.paymentId),
    );
          ...
          <div className="lg:px-16 px-5 py-20 text-white min-h-screen max-h-full w-full">
            ...
            <div className="flex justify-between items-baseline">
              ...
              <div className="text-center text-black bg-white rounded-lg p-2 md:p-4">
                <h1 className="uppercase text-[12px] md:text-sm">
                  Số vé hiện tại bạn đã đặt
                </h1>
                <p className="font-bold">
                  {
                    newReservations.filter((item) => item.author._id === userId)
                      .length
                  }
                </p>
                <p className="text-[9px] md:text-[11px] font-thin">
                  Bạn chỉ hủy vé trong 12h sau khi đặt thành công
                </p>
              </div>
            </div>
